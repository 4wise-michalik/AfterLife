...
const saveData = () => {
  addPost(JSON.parse(sessionStorage.getItem("userData"))[0].id, props.platform_id, content.value, hours.value);
  ...
  <div v-if="isPopupOpen" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-10" @click="closePopup">
    <div>
      <div @click.stop class="bg-gray-400 p-6 rounded-lg w-96">
        ...
        <input type="range" v-model="hours" min="0" max="100" step="1" class="slider" />
        {{ hours }}
