...
onMounted(() => {
  const firstName = sessionGetUserData().first_name;
  const lastName = sessionGetUserData().last_name;
...
const logOut = () => {
  sessionStorage.removeItem("authToken"); // usuwa informacje o userze i sesji
  navigateTo("/");
};
</script>
